...
var round_robin_listenPort = 5050;
var sticky_iphash_listenPort = 5051;
var addresses = [
	{target:'http://172.20.36.201:80'}
    // {
...
var rrServer = http.createServer(function (req, res) {
	console.log(req.url+' redirected to '+JSON.stringify
(addresses[i]));
	rrProxy.web(req, res, addresses[i]);
	i = (i + 1) % addresses.length;
});
rrServer.listen(round_robin_listenPort);
console.log("-start round robin loadbalancer on port 
"+round_robin_listenPort);
...
console.log("-start sticky(ip-hash) loadbalancer on port 
"+sticky_iphash_listenPort);